...
class CellMerger(BaseGSONMerger):
    def __init__(
        ...
        """Merge cell segmentation geojson tile-files
        ...
        """
        super().__init__(in_dir, xmax, ymax, xmin, ymin, tile_size)
    ...
    def _handle_multipoly(self, multipoly: MultiPolygon) -> List[Polygon]:
        ...
        try:
            ...
            warnings.warn(
                "Failed to explode and merge a multipolygon. Continue", RuntimeWarning
            )

        return ret

    def _get_coords(
        self, poly: Union[List[shapely.geometry.Polygon], shapely.geometry.Polygon]
    ...
    def _merge_adj_ploygons(
        ...
        """Merge adjascent geojsons.

        This combines the cells that are split at the img borders.

        Parameters
        ----------
            gson : GSONTile
                Geojson tile of the main tile.
            gson_adj : GSONTile
                Geojson tile of the adjascent tile.
            adj_pos : str
        ...
        -------
            List[Dict[str, Any]]:
                A list of geojson formatted dictionaries of the combined cell
                annotations.
        """
            ...
            for main_poly, c in zip(
                ...
                for adj_poly in border_annots_adj.geometry:
                    ...
                    if main_poly.intersects(adj_poly):
                        ...
                        # do some simplifying
                        new_poly = (
                            new_poly.buffer(10.0, join_style=1)
                            .buffer(-10.0, join_style=1)
                            .simplify(0.3)
                        )
    ...
    def _get_non_border_polygons(self, gson: GSONTile) -> List[Dict[str, Any]]:
        """Get all the polygons of a tile that are not adjascent to polygons in other
        tiles and make them valid.
        ...
        -------
            List[Dict[str, Any]]:
                A list of geojson formatted dictionaries of the combined cell annots
        """
    ...
    def merge(self, fname: str) -> None:
        ...
        NOTE:
        This does not handle corners so there will be some small number of overlapping
        corner cells. However, rhey have a neglible effect on the results.
