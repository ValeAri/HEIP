...
class AreaMerger(BaseGSONMerger):
    def __init__(
        ...
        """Merge area segmentation geojson tile-files

        NOTE: File names need to contain x- and y- coordinates.
        ...
        self.tiles = [GSONTile(f, *self._get_xy_coords(f.name)) for f in self.files]
    ...
    def merge(self, fname: Union[str, Path]) -> None:
        """Merge the area/semantic predictions
        ...
        for _, row in pbar:
            ...
            if isinstance(geo, shapely.geometry.MultiPolygon):
                ...
                new_coords = []
                tree = STRtree([poly.buffer(1.0) for poly in geo])
